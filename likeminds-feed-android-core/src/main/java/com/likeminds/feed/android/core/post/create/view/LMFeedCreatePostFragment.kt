...
open class LMFeedCreatePostFragment : Fragment(), LMFeedUniversalFeedAdapterListener {
    ...
    protected open fun customizePostPollAttachment(pollView: LMFeedPostPollView) {
        var postMediaStyles = LMFeedStyleTransformer.postViewStyle.postMediaViewStyle

        val updatedPollStyles = getUpdatedPollViewStyles()

        postMediaStyles = postMediaStyles.toBuilder()
            .postPollMediaStyle(updatedPollStyles)
            .build()

        val updatedPostStyles = LMFeedStyleTransformer.postViewStyle.toBuilder()
            .postMediaViewStyle(postMediaStyles)
            .build()

        LMFeedStyleTransformer.postViewStyle = updatedPostStyles
        pollView.setStyle(updatedPollStyles)
    }

    private fun getUpdatedPollViewStyles(): LMFeedPostPollViewStyle {
